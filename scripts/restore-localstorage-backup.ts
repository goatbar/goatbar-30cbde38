...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  console.error(
    "❌ Defina SUPABASE_URL/SUPABASE_ANON_KEY (ou VITE_*) no ambiente antes de executar.",
  );
  process.exit(1);
}
...
async function upsertWithMissingColumnTolerance(table: string, rows: AnyRecord[]) {
  ...
  while (attempt < 20) {
    ...
    const hadColumn = sanitizedRows.some((row) =>
  ...
  throw new Error(
    `Erro no upsert em ${table}: não foi possível estabilizar payload após múltiplas tentativas.`,
  );
}
...
async function importByUpsert(
  ...
  if (!validRows.length)
    return { inserted: 0, updated: 0, ignored: ignoredRows, total: rows.length };
...
async function run() {
  ...
  const drinks: AnyRecord[] = [];
  let drinksIgnored = 0;
  drinksRaw.forEach((row, idx) => {
    const { payload, ignoredCount } = pickMapped(
      row,
      {
        id: ["id"],
        name: ["name", "nome"],
        cost: ["cost", "custoUnitario"],
        price: ["price", "precoVenda"],
    ...
    drinks.push(payload);
    drinksIgnored += ignoredCount;
  });

  const inventory: AnyRecord[] = [];
  let inventoryIgnored = 0;
  inventoryRaw.forEach((row, idx) => {
    const { payload, ignoredCount } = pickMapped(
      row,
      {
        id: ["id"],
        ...
        quantity: ["quantity", "quantidadeTotal"],
        unit: ["unit", "unidade"],
        cost_per_unit: ["cost_per_unit", "costPerUnit", "custoUnitario"],
    ...
    payload.cost_per_unit = toNumber(payload.cost_per_unit);
    inventory.push(payload);
    inventoryIgnored += ignoredCount;
  });

  const events: AnyRecord[] = [];
  let eventsIgnored = 0;
  eventsRaw.forEach((row, idx) => {
    const { payload, ignoredCount } = pickMapped(
      row,
      {
        id: ["id"],
        client_name: ["client_name", "nome", "clienteNome"],
        date: ["date", "data"],
        event_type: ["event_type", "tipo"],
        guests: ["guests", "convidados"],
        phone: ["phone", "telefone"],
        email: ["email"],
        event_time: ["event_time", "horario"],
        event_location: ["event_location", "local"],
        city: ["city", "cidade"],
        notes: ["notes", "observacoes"],
        status: ["status"],
        is_paid_full: ["is_paid_full", "isPaidFull"],
        payment_due_date: ["payment_due_date", "paymentDueDate"],
        payment_percent_received: ["payment_percent_received", "paymentPercentReceived"],
        current_budget_value: ["current_budget_value", "currentBudgetValue"],
        current_profit_value: ["current_profit_value", "currentProfitValue"],
      },
      "events",
      idx,
    );
    payload.guests = toNumber(payload.guests);
    payload.payment_percent_received = toNumber(payload.payment_percent_received);
    payload.current_budget_value = toNumber(payload.current_budget_value);
    payload.current_profit_value = toNumber(payload.current_profit_value);
    events.push(payload);
    eventsIgnored += ignoredCount;
  });

  const sales: AnyRecord[] = [];
  const salesItems: AnyRecord[] = [];
  let salesIgnored = 0;
  let salesItemsIgnored = 0;
  for (const [idx, saleRow] of salesRaw.entries()) {
    const saleMapped = pickMapped(
      saleRow,
      {
        id: ["id"],
        date: ["date", "data"],
        location: ["location", "modality", "modalidade"],
        total_revenue: ["total_revenue", "faturamentoTotal", "totalRevenue"],
        total_cost: ["total_cost", "custoTotal", "totalCost"],
        total_profit: ["total_profit", "lucroTotal", "totalProfit"],
      },
      "sales",
      idx,
    );
    ...
    for (const [itemIdx, itemRow] of asArray<AnyRecord>(saleRow.items).entries()) {
      ...
      salesItems.push(itemMapped.payload);
      salesItemsIgnored += itemMapped.ignoredCount;
    }
  }

  const sessions: AnyRecord[] = [];
  const sessionItems: AnyRecord[] = [];
  let sessionsIgnored = 0;
  let sessionItemsIgnored = 0;
  for (const [idx, sessionRow] of sessionsRaw.entries()) {
    const sessionMapped = pickMapped(
      sessionRow,
      {
        id: ["id"],
        date: ["date", "data"],
        modality: ["modality", "modalidade"],
        labor_value: ["labor_value", "maoDeObraValor"],
        labor_quantity: ["labor_quantity", "maoDeObraQtd"],
        labor_names: ["labor_names", "maoDeObraNomes"],
        labor_details: ["labor_details", "maoDeObraDetalhes"],
        created_at: ["created_at", "createdAt"],
        updated_at: ["updated_at", "updatedAt"],
      },
      "financial_sessions",
    ...
    sessions.push(sessionMapped.payload);
    sessionsIgnored += sessionMapped.ignoredCount;
    ...
    for (const [itemIdx, itemRow] of asArray<AnyRecord>(sessionRow.items).entries()) {
      const itemMapped = pickMapped(
        itemRow,
        {
          ...
          drink_id: ["drink_id", "drinkId"],
          drink_name: ["drink_name", "nome"],
          quantity: ["quantity", "quantidade"],
          unit_price: ["unit_price", "precoUnitario"],
          unit_cost: ["unit_cost", "custoUnitario"],
          ingredient_cost: ["ingredient_cost", "custoInsumo"],
        },
        "financial_session_items",
        itemIdx,
      );
      ...
      sessionItems.push(itemMapped.payload);
      sessionItemsIgnored += itemMapped.ignoredCount;
    }
  }
  ...
  const preview = {
    ...
    financial_session_items: await importByUpsert(
      "financial_session_items",
      sessionItems,
      sessionItemsIgnored,
      true,
  ...
  Object.entries(preview).forEach(([table, stat]) =>
    console.log(
      `${table}: inseridos=${stat.inserted}, atualizados=${stat.updated}, ignorados=${stat.ignored}`,
    ),
  );

  const rl = readline.createInterface({ input, output });
  const confirmation = (
    await rl.question("Digite CONFIRMAR_IMPORTACAO para gravar no Supabase: ")
  ).trim();
  rl.close();
  if (confirmation !== "CONFIRMAR_IMPORTACAO")
    return console.log("Importação cancelada. Nenhum dado foi gravado.");
  ...
  const finalStats = [
    ...
    [
      "financial_sessions",
      await importByUpsert("financial_sessions", sessions, sessionsIgnored, false),
    ],
    [
      "financial_session_items",
      await importByUpsert("financial_session_items", sessionItems, sessionItemsIgnored, false),
    ],
  ] as const;
