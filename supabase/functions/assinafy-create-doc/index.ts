...
import { uploadDocument, findSigner, createSigner, createAssignment } from "../_shared/assinafy-client.ts";
  ...
  try {
    ...
    if (sigReq && sigReq.dispatch_status === "processing") {
        ...
        if (diffMs > 10 * 60 * 1000) {
            await supabaseAdmin.from("contract_signature_requests")
                .update({ dispatch_status: "reconciliation_required", last_error: "Timeout na tentativa anterior. Reconciliação manual necessária." })
                .eq("id", sigReq.id);
            throw new Error("Falha opaca no envio anterior. Por segurança o envio automático foi bloqueado (reconciliation_required).");
        } else {
    ...
    if (sigReq && sigReq.dispatch_status === "reconciliation_required") {
        throw new Error("Envio bloqueado por segurança devido a falha opaca anterior. Reconciliação manual necessária.");
    }
    ...
    try {
        ...
        if (!externalDocId) {
            ...
            await supabaseAdmin.from("contract_signature_requests").update({ external_document_id: externalDocId }).eq("id", lockedReq.id);
        }
        ...
        if (!externalAssignId) {
            ...
            const sigUrl = assignRes?.data?.signature_url || assignRes?.signature_url;
