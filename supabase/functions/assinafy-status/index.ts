...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "sync") {
      ...
      if (!sigReq) {
        return new Response(JSON.stringify({ status: "not_sent" }), {
      ...
      const { data: signers } = await supabaseAdmin
        ...
        .select("full_name,email,status,signature_url,notification_status,notified_at,signed_at")
      ...
      if (
        ...
        ["failed", "reconciliation_required", "canceled"].includes(sigReq.dispatch_status)
      ) {
        return new Response(JSON.stringify({ ...localDetails, status: sigReq.dispatch_status }), {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      if (apiStatus !== sigReq.dispatch_status) {
        await supabaseAdmin
          ...
          .update({
            ...
            internal_status:
              apiStatus === "completed" || apiStatus === "signed" ? "signed" : "pending_signature",
          })
          .eq("id", sigReq.id);
      }
      ...
      const contentDisposition =
        res.headers?.get("Content-Disposition") || `attachment; filename="${artifact}.pdf"`;
