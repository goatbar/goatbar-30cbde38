...
function Dashboard() {
  ...
  const proximosPagamentos = [...eventosSupabase]
    ...
    .map((e) => {
      ...
      return {
        ...e,
        percentualPago: percentPago,
        percentualPendente: percentPendente,
        valorPendente: pendente,
        dataVencimentoPagamento: vencimento,
      };
    })
      ...
      <div className="page-container space-y-6 lg:space-y-7">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 sm:gap-5">
          ...
          <StatCard
            ...
            value={String(metrics.events.count)}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
          <SectionCard
            title="Drinks mais rentáveis"
            subtitle="Participação no lucro por sessões diretas"
          >
          ...
          <SectionCard
            ...
            subtitle="Agenda operacional"
            action={
              <Link to="/eventos" className="text-xs text-primary font-medium">
                Ver todos
              </Link>
            }
          >
            <div className="space-y-2">
              {proximosEventos.map((e) => (
                <Link
                  ...
                  className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-surface/50 transition-all group"
                >
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm truncate">{e.nome || e.client_name}</div>
                    <div className="text-xs text-muted-foreground mt-0.5">
                      {e.date ? format(parseISO(e.date), "dd/MMM", { locale: ptBR }) : "--"} ·{" "}
                      {e.guests || e.convidados} convidados
                    </div>
                    <div className="text-[11px] text-muted-foreground/90 mt-0.5 truncate">
                      {[e.event_type, e.city].filter(Boolean).join(" · ") ||
                        "Tipo/cidade não informados"}
                    </div>
                  </div>
          ...
          <SectionCard
            ...
            subtitle="Fluxo de caixa de eventos confirmados"
            className="border-amber-500/20 shadow-lg shadow-amber-500/5"
          >
            <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] font-bold text-amber-600 uppercase tracking-widest mt-0.5">
                      Vence em:{" "}
                      {e.dataVencimentoPagamento
                        ? format(parseISO(e.dataVencimentoPagamento), "dd/MM/yyyy")
                        : "A definir"}
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-sm font-black text-foreground">
                      {fmtBRL(e.valorPendente)}
                    </div>
                    <div className="text-[9px] font-bold text-muted-foreground uppercase">
                      {e.percentualPendente.toFixed(0)}% pendente · {e.percentualPago.toFixed(0)}%
                      pago
                    </div>
                  </div>
                </Link>
        ...
        <SectionCard title="Resumo por Modalidade" subtitle="Consolidado de resultados líquidos">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <SummaryBlock
              ...
              receita={metrics.bot.receita}
              lucro={metrics.bot.lucro}
            />
            <SummaryBlock
              title="7Steakhouse"
              icon={<Calculator className="h-4 w-4 text-success" />}
              receita={metrics.steak.receita}
              lucro={metrics.steak.lucro}
            />
            <SummaryBlock
              title="Eventos Fechados"
              icon={<CalendarRange className="h-4 w-4 text-amber-500" />}
              receita={metrics.events.receita}
              lucro={metrics.events.lucro}
            />
          </div>
        </SectionCard>
      </div>
