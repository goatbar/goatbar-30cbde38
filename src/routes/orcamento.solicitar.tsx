...
function PublicBudgetRequestPage() {
  ...
  const captureContact = () => {
    if (!context || !form.client_name.trim() || form.phone.replace(/\D/g, "").length < 10) return;
    budgetRequestService
  ...
  const field = (label: string, key: keyof BudgetRequestPayload, type = "text", required = false) => (
    <label className="space-y-2 text-sm font-medium">
      ...
      <input
        ...
        onBlur={key === "client_name" || key === "phone" || key === "email" ? captureContact : undefined}
        onChange={(e) =>
          setForm((old) => ({
            ...old,
        ...
        <form onSubmit={submit} className="grid gap-5 sm:grid-cols-2">
          ...
          <label className="space-y-2 text-sm font-medium">
            ...
            <select
              ...
              className="mt-2 h-11 w-full rounded-lg border border-border bg-input px-3"
            >
              {[3, 4, 5, 6, 7, 8].map((hours) => <option key={hours} value={hours}>{hours} horas</option>)}
            </select>
          </label>
          ...
          <label className="space-y-2 text-sm font-medium">
            ...
            >
              ...
              {["Instagram", "Google", "WhatsApp", "Indicação"].map((source) => <option key={source}>{source}</option>)}
            </select>
          </label>
            ...
            <fieldset className="sm:col-span-2 min-w-0 space-y-4 border-t border-border pt-6">
              ...
              <div className="grid min-w-0 grid-cols-1 gap-4 md:grid-cols-3">
                {publicDrinks.map((drink) => {
                  ...
                  return (
                    <label key={drink.id} className={`min-w-0 cursor-pointer overflow-hidden rounded-xl border bg-background transition ${selected ? "border-primary ring-1 ring-primary" : "border-border"}`}>
                      {drink.image ? (
                        <img src={drink.image} alt="" className="h-40 w-full object-cover" />
                      ) : (
                        <div className="flex h-40 items-center justify-center bg-input text-muted-foreground"><ImageOff className="h-8 w-8" /></div>
                      )}
                      <div className="space-y-2 p-4">
                        ...
                        <span className="flex items-center gap-2 text-sm font-medium">
                          <input type="checkbox" checked={selected} onChange={() => toggleDrink(drink.id)} className="h-4 w-4 accent-primary" />
                          Quero este drink
                        </span>
                      </div>
                    </label>
          ...
          <p className="sm:col-span-2 text-xs text-muted-foreground">
            Ao enviar, você autoriza a Goat Bar a utilizar estes dados para entrar em contato sobre o seu evento e esta solicitação de orçamento.
          </p>

          {error && <p role="alert" className="sm:col-span-2 text-sm text-destructive">{error}</p>}

          <button disabled={submitting} className="sm:col-span-2 inline-flex h-12 items-center justify-center rounded-lg bg-primary font-bold text-primary-foreground disabled:opacity-60">
