                  ...
                  {contracts.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {eventContracts.map((ec) => {
                        ...
                        const client = eventContractClientDatas.find(
                          (c) => c.eventId === ec.eventId,
                        );
                        ...
                        return (
                          <div
                            key={ec.id}
                            className="p-5 border border-border bg-surface rounded-xl flex flex-col md:flex-row justify-between gap-5"
                          >
                            <div>
                              <div className="flex items-center gap-3 mb-1">
                                ...
                                <span
                                  className={`px-2 py-0.5 rounded text-[10px] uppercase font-medium tracking-wider ${ec.status === "assinado" ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}
                                >
                                  {ec.status.replace("_", " ")}
                                </span>
                              </div>
                              ...
                              <div className="text-xs text-muted-foreground mt-2">
                                Template: {template?.name} (v{ec.version})
                              </div>
                            </div>
                            <div className="flex flex-col gap-2 justify-center">
                              <GhostButton className="h-8 text-xs">
                                <Download className="h-3 w-3" /> Contrato PDF
                              </GhostButton>
                              {ec.status === "assinado" && (
                                <GhostButton className="h-8 text-xs text-success hover:bg-success/10">
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {templates.map((t) => (
                      <div
                        key={t.id}
                        className="p-4 border border-border rounded-xl bg-surface relative group"
                      >
                        ...
                        <div className="mt-3 flex gap-1 flex-wrap">
                          {t.variables_schema?.slice(0, 3).map((v) => (
                            <span
                              key={v}
                          ...
                          {t.variables_schema?.length > 3 && (
                            <span className="text-[10px] px-1.5 py-0.5 bg-background border border-border rounded">
                              +{t.variables_schema.length - 3}
                            </span>
                          )}
                        </div>
                    ...
                    {templates.length === 0 && (
                      <div className="col-span-2 text-center py-10 text-muted-foreground text-sm border border-dashed border-border rounded-xl">
                ...
                >
                  <div className="space-y-3">
                    {signers.map((s) => (
                      <div
                        key={s.id}
                        className={`p-4 border rounded-xl flex justify-between items-center ${s.is_active ? "border-border bg-surface" : "border-border/50 bg-background opacity-60"}`}
                      >
                        <div>
                        ...
                        <div>
                          <span
                            className={`text-[10px] uppercase font-medium px-2 py-0.5 rounded ${s.is_active ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}`}
                          >
                ...
                <SectionCard
                  ...
                  action={
                    <PrimaryButton
                      ...
                      className="h-9 px-3 text-sm"
                    >
                      <Plus className="h-4 w-4" /> Novo Copo
                    </PrimaryButton>
                  }
                >
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-left">
                          ...
                          <th className="label-eyebrow px-6 py-3 border-y border-border">
                            Reposição
                          </th>
                        ...
                        {glasswareList.map((g) => (
                          <tr key={g.id} className="border-b border-border/60">
                            ...
                            <td className="px-6 py-3.5">
                              <span
                                className={`text-[10px] uppercase font-medium px-2 py-0.5 rounded ${g.is_active ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}`}
                              >
              ...
              <PrimaryButton
                onClick={async () => {
                  if (!newTemplate.name || !selectedFile)
                    return alert("Preencha o nome e selecione o arquivo.");
                  setUploading(true);
                  try {
                    const { publicUrl, filePath } =
                      await contractTemplatesService.uploadTemplateFile(selectedFile);
